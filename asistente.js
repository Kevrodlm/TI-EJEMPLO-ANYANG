if (annyang) {
    ...
    var commands = {
        ...
        'donde queda mi escuela': function () {
            utter.text = 'claro, pero dime,  cual es tu carrera?';
            utter.voice = voices[0];
            window.speechSynthesis.speak(utter);
            //Guarda el nombre que le decimos por voz.
            annyang.addCallback('result', function (phrases) {
                ...
                annyang.removeCallback('result');
                //Nos dice hola + el nombre que le digamos por voz.
                utter.text = 'La facultad de la escuela de ' + phrases[0]+ 'si entras por la puerta paucarpata, sigue recto hasta la cancha de mecanica luego gira a la derecha en dirección a la biblioteca y sigue ese camino sin para y e ncontraras tu facultad';
                window.speechSynthesis.speak(utter);
            });
        },
